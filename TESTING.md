...
- unit behavior: table-driven Go tests close to the code being changed
...
- scale checks: large-repo fixtures and the performance gate (`moon run :perf`, asserted in `internal/perf`)
...
Every behavior change proves itself before it lands:

- a failing test or failing fixture-driven assertion first
- the smallest implementation required to pass it
- one exact verification command recorded with the change
- rule spec or companion doc updated if behavior changed
- no hidden expansion from one rule into many unrelated concerns

## CLI Quality

Treat quiet mode, redaction, rule labeling, and machine-readable output as product surfaces, not incidental logging details. Styled (TTY) and plain (non-TTY) paths are both contracts — verify each.
