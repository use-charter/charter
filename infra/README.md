...
```
                 ┌──────────────────────────── use-charter.dev ────────────────────────────┐
   request ──▶  charter-router  ──┬─ /docs/*, /rules/*  ──▶  charter.mintlify.dev
...
- Authenticate: `wrangler login`, or set `CLOUDFLARE_API_TOKEN` (+ `CLOUDFLARE_ACCOUNT_ID`)
  for CI. Account: `Tashfiq61@gmail.com` (`9b7d6fadbaeb947d19e41decc1c56881`).
  Zone `use-charter.dev` = `3e04f444539a35fe073ba1947c612b1b`.
...
2. **`charter-router`** — `bun run deploy:router`, then in the dashboard set
   ...
   `MINTLIFY_ORIGIN` (defaults to `charter.mintlify.dev`). The `use-charter.dev/*`
