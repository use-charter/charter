...
async function build(db: D1Database): Promise<string> {
	...
	const [pv, uq, tp, blog, docs, copies, geo] = await db.batch([
		q(
			"SELECT day, SUM(hits) AS count FROM pageview WHERE day >= ? GROUP BY day ORDER BY day",
		),
		q(
			"SELECT day, COUNT(*) AS count FROM visitor WHERE day >= ? GROUP BY day ORDER BY day",
		),
		q(
			"SELECT path, SUM(hits) AS hits FROM pageview WHERE day >= ? GROUP BY path ORDER BY hits DESC LIMIT 10",
		),
		q(
			"SELECT COALESCE(SUM(hits), 0) AS n FROM pageview WHERE day >= ? AND (path = '/blog' OR path LIKE '/blog/%')",
		),
		q(
			"SELECT COALESCE(SUM(hits), 0) AS n FROM pageview WHERE day >= ? AND path = '/docs/*'",
		),
		q(
			"SELECT COUNT(*) AS n FROM event WHERE day >= ? AND type = 'install_copied'",
		),
		q(
			"SELECT country, SUM(hits) AS hits FROM geo WHERE day >= ? GROUP BY country ORDER BY hits DESC LIMIT 5",
		),
	]);
	...
	return JSON.stringify({
		...
		pageviewsByDay: (pv.results as DayCount[]).map((r) => ({
			day: r.day,
			count: Number(r.count),
		})),
		uniquesByDay: (uq.results as DayCount[]).map((r) => ({
			day: r.day,
			count: Number(r.count),
		})),
		topPages: (tp.results as PathHits[]).map((r) => ({
			path: r.path,
			hits: Number(r.hits),
		})),
		...
		topCountries: (geo.results as CountryHits[]).map((r) => ({
			country: r.country,
			hits: Number(r.hits),
		})),
	});
