...
async function buildStats(token: string): Promise<unknown> {
	...
	const repo = await group(
		() => gh<RepoResp>(`/repos/${REPO}`, token),
		errors,
		"growth",
	);
	const views = await group(
		() => gh<TrafficResp>(`/repos/${REPO}/traffic/views`, token),
		errors,
		"traffic_views",
	);
	const clones = await group(
		() => gh<TrafficResp>(`/repos/${REPO}/traffic/clones`, token),
		errors,
		"traffic_clones",
	);
	const releases = await group(
		() => gh<ReleaseResp[]>(`/repos/${REPO}/releases?per_page=10`, token),
		errors,
		"releases",
	);
	const actionUse = await group(
		() =>
			gh<SearchResp>(
				`/search/code?q=${encodeURIComponent('"use-charter/charter-action"')}&per_page=20`,
				token,
			),
		errors,
		"adoption_action",
	...
	const schemaUse = await group(
		() =>
			gh<SearchResp>(
				`/search/code?q=${encodeURIComponent('"use-charter.dev/schema/charter.schema.json"')}&per_page=20`,
				token,
			),
		errors,
		"adoption_schema",
	...
	const openPRs = await group(
		() =>
			gh<SearchResp>(
				`/search/issues?q=${encodeURIComponent(`repo:${REPO} type:pr state:open`)}&per_page=1`,
				token,
			),
		errors,
		"community_prs",
	...
	return {
		...
		releases: releases &&
			releases.length > 0 && {
				...
				// releases[0] and its assets are guaranteed here (length > 0, and the
				// reduce above already dereferences assets), so the ?? [] never runs.
				/* v8 ignore next */
				assets: (releases[0]?.assets ?? []).map((a) => ({
					name: a.name,
...
export async function handleDashboardStats(
		...
		return json(
			{
				...
				message:
					"Set the GITHUB_STATS_TOKEN worker secret to enable the dashboard.",
			},
			200,
		);
	}
	...
	// buildStats always returns an `errors` object, so the ?? {} never runs.
	/* v8 ignore next */
	const hadErrors =
		Object.keys((stats as { errors?: Record<string, string> }).errors ?? {})
			.length > 0;
	...
	const res = json(stats, 200, {
		"Cache-Control": hadErrors ? "no-store" : `public, max-age=${CACHE_TTL}`,
	});
