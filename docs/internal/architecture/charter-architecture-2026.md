...
| AE-TEST-001 | Testing | Every active code language (manifest present + non-test source outside tooling dirs) has automated tests, so an agent can verify its own work; tooling-only manifests and stray secondary-language files are N/A (ADR-0023) | **High** |
