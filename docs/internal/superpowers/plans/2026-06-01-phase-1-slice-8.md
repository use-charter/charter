...
**Architecture:** A pure `internal/render/sarif` renderer (mirroring `render/json`) consumes a new pure `internal/rules/catalog` (rule metadata) and `internal/version.Version()`. `internal/config` gains `policy` loading + a threshold resolver; `internal/doctor.Run` resolves the effective threshold. `cmd/charter/doctor.go` adds `--format sarif` and `--out`. Deterministic, offline. **No new runtime dependencies** (runtime uses stdlib `crypto/sha256`/`encoding/json` + existing `gopkg.in/yaml.v3`); the one new dependency is **test-only** — `github.com/santhosh-tekuri/jsonschema/v6` (pinned v6.0.2) validates emitted SARIF against the official schema and never enters the shipped binary (ADR-0014).

**Tech Stack:** Go 1.26.3, Cobra, `findings`/`scoring`/`repository`/`doctor`/`suppress` seams, `gopkg.in/yaml.v3`, `crypto/sha256` + `encoding/json` + `runtime/debug` (stdlib), test-only `github.com/santhosh-tekuri/jsonschema/v6` (SARIF schema validation), fixtures under `testdata/`, root verification via `moon run :check`. Module path: `go.use-charter.dev/charter`.
...
- [ ] **Step 1: Add a `Version()` func** (2026 idiom — `runtime/debug` build info with an ldflags fallback, so `go install …@vX` is accurate without ldflags). Create `internal/version/version.go`:
...
Add a `version_test.go` asserting `Version()` is non-empty and defaults to `0.0.0-dev` in the test build (no ldflags, module version `(devel)`).
...
Extend the test (or add one) to also assert: `runs[0].automationDetails.id == "charter"`; a non-informational rule (e.g. `AE-SEC-001`) has `properties["security-severity"] == "9.5"` and `properties.tags` contains `"security"`; the informational rule (`AE-SUPPRESS-003`) has neither `security-severity` nor `tags`.
...
- [ ] **Step 5: Add SARIF schema-conformance + golden tests (test-only dependency).**
  - Add the pinned validator: `mise exec -- go get github.com/santhosh-tekuri/jsonschema/v6@v6.0.2`.
  - Vendor the official schema once (then committed for offline tests): download `https://json.schemastore.org/sarif-2.1.0.json` → `internal/render/sarif/testdata/sarif-2.1.0.schema.json`.
  - Write `internal/render/sarif/render_schema_test.go` validating a rendered document (use a sample result with active + suppressed + informational findings) against the schema. **Confirm the v6 API via its godoc/README**; the documented idiom is:
...
func TestRenderConformsToSARIFSchema(t *testing.T) {
	data, err := Render(sampleResult(t))
	if err != nil {
		t.Fatal(err)
	}
	schemaBytes, err := os.ReadFile("testdata/sarif-2.1.0.schema.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := sch.Validate(inst); err != nil {
		t.Fatalf("SARIF output does not conform to sarif-2.1.0: %v", err)
	}
}
```

  - Add a golden-file test: render a fixed sample result and compare bytes to a committed `internal/render/sarif/testdata/golden.sarif.json` (regenerate intentionally when the mapping changes; a `-update` flag is optional). Run `mise exec -- go test ./internal/render/sarif -race -count=1`, then `mise exec -- go mod tidy` and confirm `go.mod`/`go.sum` pin `v6.0.2`.

- [ ] **Step 6: Verify** `go vet`, `gofumpt -l internal/render/sarif` clean; `mise exec -- go test ./internal/render/sarif -cover` (≥85%); `mise exec -- moon run :security` clean (osv-scanner/govulncheck now also see the test dep). Do NOT commit.
...
1. Hand-rolled `internal/render/sarif` (no SARIF **runtime** dependency); SARIF **2.1.0**, validated in tests against the vendored official schema via a pinned **test-only** `santhosh-tekuri/jsonschema/v6` plus a golden-file test. Rules carry `security-severity` + `tags:["security"]` (non-informational); the run sets `automationDetails.id: "charter"`; version via `version.Version()` (`runtime/debug` + ldflags fallback).
