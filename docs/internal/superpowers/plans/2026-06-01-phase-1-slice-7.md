...
- [ ] **Step 3: Wire the pipeline** in `internal/doctor/run.go`. Add imports `"time"`, `gogovernance "go.charter.dev/charter/internal/rules/governance"`, and `"go.charter.dev/charter/internal/suppress"`. Add `Suppressed []suppress.Suppressed` to `Result`. After `all` is fully assembled and before `scoring.Calculate`, replace the score/return block with:
...
Import `"go.charter.dev/charter/internal/suppress"`. Informational findings already render in the findings table (no special-casing needed); add a renderer test for the suppressed section.
