...
- **Outputs:** `exit-code`, `score`, and `sarif-file` (for downstream steps). `score` is derived from a fast follow-up JSON scan so downstream workflows can gate or annotate on the numeric result without parsing SARIF.
