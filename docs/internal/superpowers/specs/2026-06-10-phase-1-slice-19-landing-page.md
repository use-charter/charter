...
- Vendor compatibility icons: all 8 SVGs confirmed in `docs/product/images/icons/`. Render all as `<img>` with proper alt text. Exact filenames: `claude-ai.svg`, `chatgpt.svg`, `grok.svg`, `cursor.svg`, `windsurf.svg`, `github-copilot.svg`, `google-gemini.svg`, `codex.svg`
...
**Worker routing:** Existing Cloudflare Worker deployed externally (no `wrangler.toml` in repo — see Critical Dependencies §4). Update via Cloudflare dashboard or `wrangler deploy` to proxy `/` to `LANDING_ORIGIN`. Existing `/docs/*` and `/rules/*` routes to Mintlify remain unchanged.

**Monorepo:** `web/` is a Moon project with `web/moon.yml` already committed and configured (dev, build, check tasks already present).
...
**Font strategy:**
- Self-hosted via base64 data URIs embedded in `web/src/styles/fonts.css` (no separate font files, no CDN)
- `font-display: swap` set in each `@font-face` block
- **No `<link rel="preload" as="font">` needed** — base64 fonts have no URL to preload; browser encounters them when `fonts.css` is parsed; load `fonts.css` early in `<head>`
- Latin subset only (generated by `bun scripts/generate-report-fonts.ts`)
...
| cursor icon | `docs/product/images/icons/cursor.svg` | ✅ Exists |
| windsurf icon | `docs/product/images/icons/windsurf.svg` | ✅ Exists |
| copilot icon | `docs/product/images/icons/github-copilot.svg` | ✅ Exists (filename: `github-copilot.svg`) |
| gemini icon | `docs/product/images/icons/google-gemini.svg` | ✅ Exists (filename: `google-gemini.svg`) |
| codex icon | `docs/product/images/icons/codex.svg` | ✅ Exists |
...
<meta property="og:url" content="https://use-charter.dev/" />
...
- [ ] All meaningful `<img>` and `<picture>` elements have non-empty, descriptive `alt` text; decorative images (e.g., TerminalCard screenshot reinforcement) have `alt=""` — verify intentional empties are actually decorative
...
**Assets & Brand**
- [ ] Fonts self-hosted as base64 in `web/src/styles/fonts.css`; no external CDN URL (`fonts.googleapis.com` or similar) in `dist/index.html`
