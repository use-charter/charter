...
|---|---|---|---|---|
| CF-4 | ~~`go install go.use-charter.dev/charter/cmd/charter@…` won't resolve until `go.use-charter.dev` serves a `go-import` meta tag.~~ **Code-closed (Slice 19, ADR-0026):** the `charter-go-vanity` worker (`infra/go-vanity/`) serves the `go-import` meta and type-checks. Resolution now needs only `wrangler deploy` + the `go.` DNS (auto-created by the `custom_domain` route). **Deployed + verified live 2026-06-13:** `go.use-charter.dev/charter?go-get=1` serves the `go-import`/`go-source` meta (TLS live). Full `go install` resolves once the repo is public + `v1.0.0` tagged. | Resolved. | Slice 19 deploy (`infra/`) | Slice 9 |
| CF-9 | ~~SARIF `helpUri`s point at `https://use-charter.dev/rules/AE-*`, dead until the rule docs pages are **deployed**.~~ **Resolved (2026-06-14):** apex flip done — `charter-router` worker + apex proxied AAAA + route `use-charter.dev/*` deployed; `https://use-charter.dev/rules/AE-MCP-001` returns 200 (Mintlify). `helpUri`s now resolve. | Resolved. | Apex flip (CF API) | Slice 8 |
| CF-10 | ~~`use-charter` GitHub org hardening: verify the `use-charter.dev` domain (DNS TXT) and require 2FA for members.~~ **Done (2026-06-13):** domain verified (TXT via Cloudflare) + org 2FA required. | — | go-public ops / Slice 20 checklist | Org migration |
