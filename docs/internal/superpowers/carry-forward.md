...
| CF-12 | MCP catalog curation is an ongoing founder duty. **Done in Slice 13:** T1.6.3 FP-validation run (11 real repos, **0% FP** after one fix) recorded in `docs/internal/catalog/fp-validation.md`; ~24 server entries + 60+ vendor hosts; real `mcp-server-git` CVEs added. **Still open:** version data for the behind-stable nudge is only seeded for `filesystem`; broaden the FP run and keep advisories current. | Real CVE IDs only (ADR-0021); ecosystem moves fast. | Refresh + re-run FP validation at the **Slice 17** release gate; ongoing via T1.6.2 process | Slice 13 |
| CF-13 | `AE-MCP-003` systematically flags OAuth 2.1 remote vendor servers (Sentry, Atlassian, Context7, …) that declare auth via the OAuth flow rather than a static config header — a likely FP class surfaced by the Slice 13 FP-validation run. | Out of scope for the catalog slice (AE-MCP-001/002); a faithful fix needs an AE-MCP-003 design pass (e.g. treat catalog-known OAuth hosts as auth-declared, or detect OAuth metadata). | Scoped `AE-MCP-003` refinement (own ground+grill); before the public tag if it proves noisy | Slice 13 |
