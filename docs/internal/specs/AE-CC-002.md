...
- Detection logic: reads the agent context — the `agentcontext` files (AGENTS.md, CLAUDE.md, .windsurfrules, .github/copilot-instructions.md, opencode.md, codex.md, DESIGN.md, SKILL.md), every tracked file under the `.cursor/rules/` directory, and `PERMISSIONS.md` when present. Passes when the context declares a concrete off-limits / protected-path boundary — recognized sensitive-path tokens presented as off-limits (`.env`, `secrets`, `.github/workflows`, `terraform`, `infra`, `db/migrations`, `credentials`) or an explicit reference to `PERMISSIONS.md`. Flags High when no concrete off-limits-path declaration is found across any context source. An unreadable tracked context file fails the scan fast with a wrapped error rather than being silently skipped. This is intentionally stricter than `AE-CTX-001`, which only requires a generic edit-boundary mention.
