...
> Generated from the internal rule spec. Edit `docs/internal/specs/AE-MCP-003.md`, then re-run `bun scripts/generate-rule-pages.ts`.

**Rule name:** MCPRemoteAuthDeclared
...
declare an auth header for the remote MCP server (for example `Authorization` referencing an environment variable), or switch to a local/trusted integration mode, then commit the change.
...
- ADR-0021 (catalog OAuth-host exemption)
- ADR-0011
- ADR-0006
- ADR-0009
