...
Charter scans MCP configuration files for server entries that use a remote `url` or a transport type of `http` or `sse`. For each remote server, Charter extracts the URL host and compares it against two sources:
